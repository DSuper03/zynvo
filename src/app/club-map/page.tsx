...
const ClubMapPage = () => {
  ...
  const [selectedClubId, setSelectedClubId] = useState(kolkataCollegeClubs[0]?.id ?? "");
  const [mapReady, setMapReady] = useState(false);

  const selectedClub = useMemo(
    () => kolkataCollegeClubs.find((club) => club.id === selectedClubId) ?? kolkataCollegeClubs[0],
    [selectedClubId]
  );
    ...
    const initializeMap = async () => {
      ...
      kolkataCollegeClubs.forEach((club) => {
        ...
        L.circle([club.latitude, club.longitude], {
          ...
          stroke: false,
          fillColor: club.category === "tech" ? "#22d3ee" : club.category === "business" ? "#f59e0b" : "#e879f9",
      ...
      const bounds = L.latLngBounds(kolkataCollegeClubs.map((club) => [club.latitude, club.longitude]));
  ...
  const focusKolkata = () => {
    mapRef.current?.flyTo([KOLKATA_CENTER.latitude, KOLKATA_CENTER.longitude], 12, {
      duration: 0.7,
    });
  };
    ...
    <div className="min-h-full overflow-hidden rounded-2xl border border-zinc-900 bg-[#050505] text-white shadow-2xl">
      <div className="grid min-h-[calc(100vh-3rem)] lg:grid-cols-[390px_1fr]">
        <aside className="flex min-h-[520px] flex-col border-b border-zinc-900 bg-zinc-950/80 lg:border-b-0 lg:border-r">
          <div className="space-y-5 border-b border-zinc-900 p-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="border-zinc-800 bg-zinc-900/70 text-zinc-300">
                {formatRadius(CLUB_DISCOVERY_RADIUS_METERS)} heat radius
              </Badge>
            </div>
            ...
            <div>
              ...
              <p className="mt-2 max-w-xl text-sm leading-6 text-zinc-400">
                Search Kolkata college club hubs, compare nearby clusters, and jump straight to a campus location on the map.
              </p>
            </div>

            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-lg border border-zinc-900 bg-black/50 p-3">
                <p className="text-xl font-bold text-white">{kolkataCollegeClubs.length}</p>
                <p className="mt-0.5 text-[11px] uppercase tracking-wide text-zinc-500">Clubs</p>
              </div>
              <div className="rounded-lg border border-zinc-900 bg-black/50 p-3">
                <p className="text-xl font-bold text-white">6</p>
                <p className="mt-0.5 text-[11px] uppercase tracking-wide text-zinc-500">Types</p>
              </div>
              <div className="rounded-lg border border-zinc-900 bg-black/50 p-3">
                <p className="text-xl font-bold text-white">10km</p>
                <p className="mt-0.5 text-[11px] uppercase tracking-wide text-zinc-500">Zone</p>
              </div>
            </div>
            ...
            {filteredClubs.length === 0 ? (
              <Card className="border-zinc-900 bg-black/50">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="mt-3 text-sm font-medium text-zinc-200">No matching club found</p>
                  <p className="mt-1 text-xs text-zinc-500">Try another college name, area, or tag.</p>
                </CardContent>
              </Card>
            ) : (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="line-clamp-1 text-sm font-semibold text-white">{club.clubName}</p>
                        <p className="mt-1 line-clamp-1 text-xs text-zinc-500">{club.collegeName}</p>
                      </div>
                      <Badge variant="outline" className={cn("shrink-0 capitalize", categoryStyles[club.category])}>
                        {club.category}
                      </Badge>
                    </div>
        ...
        <section className="relative min-h-[620px] bg-black">
          <div ref={mapNodeRef} className="h-full min-h-[620px] w-full" aria-label="Kolkata college club map" />
              ...
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-white">
                    {selectedClub?.clubName ?? "Kolkata club network"}
                  </p>
                  <p className="truncate text-xs text-zinc-400">
                    {selectedClub ? `${selectedClub.area} · ${selectedClub.address}` : "Explore clubs across Kolkata"}
                  </p>
                </div>
              </div>
              <button
          ...
          <div className="pointer-events-none absolute bottom-4 left-4 right-4 z-[400] grid gap-3 md:grid-cols-[1fr_auto]">
              ...
              <CardContent className="grid gap-4 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-zinc-400">
                    Larger bright areas show dense or high-activity college club zones inside the 10 km Kolkata discovery circle.
                  </p>
                </div>
            ...
            <Card className="pointer-events-auto border-zinc-800 bg-black/85 text-white backdrop-blur-md">
              <CardContent className="flex h-full items-center gap-3 p-4">
                ...
                <div>
                  <p className="text-sm font-semibold">{filteredClubs.length} visible</p>
                  <p className="text-xs text-zinc-500">Search updates map list</p>
                </div>
