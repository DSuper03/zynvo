...
const PaymentProofModal: React.FC<PaymentProofModalProps> = ({
  ...
  const AUTO_COMPRESS_BYTES = 2 * 1024 * 1024;
  ...
  const prepareProofImage = async (file: File) => {
    let processed = file;

    if (file.size > AUTO_COMPRESS_BYTES) {
      processed = await compressImageToUnder2MB(file);
    }

    if (processed.size > HARD_LIMIT_BYTES) {
      toast.error('Could not process screenshot. Try a smaller image.');
      return null;
    }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const processed = await prepareProofImage(file);
      if (!processed) {
  ...
  const handleSubmitProof = async () => {
    ...
    setIsUploading(true);
    try {
      const toUpload = await prepareProofImage(proofImage);
      if (!toUpload) {
  ...
  return (
    <>
      {/* Main Modal */}
      <div 
        className="fixed inset-0 z-[110] flex items-end justify-center p-2 pb-4 sm:items-center sm:p-4"
        onClick={(e) => {
      ...
      >
        ...
        {/* Modal Content */}
        <Card className="relative z-10 w-full max-w-lg max-h-[min(88dvh,760px)] sm:max-h-[90vh] bg-[#0a0a0a] border border-gray-800/50 shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="overflow-y-auto flex-1">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              {!proofPreviewUrl ? (
                ...
                <div className="space-y-2 sm:space-y-3">
                  <div className="relative w-full h-40 sm:h-56 rounded-xl overflow-hidden border border-gray-800/50 bg-gray-900/50">
                    <Image
                      src={proofPreviewUrl}
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <Button
                      ...
                      variant="outline"
                      onClick={openFilePicker}
                      className="flex-1 bg-gray-900/50 border-gray-700/50 hover:bg-gray-800/50 text-white text-xs sm:text-sm"
                      disabled={isUploading || isSubmitting}
                    >
                      Change Image
                    </Button>
          ...
          <div className="flex-shrink-0 border-t border-gray-800/50 bg-[#0a0a0a] p-3 sm:p-6 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:pb-6">
            <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-row sm:gap-3">
              <Button
                type="button"
                variant="outline"
                onClick={onClose}
                className="w-full bg-gray-900/50 border-gray-700/50 hover:bg-gray-800/50 text-white text-sm sm:text-base"
                disabled={isUploading || isSubmitting}
              >
                Cancel
              </Button>
              <Button
                ...
                className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-black font-semibold disabled:opacity-50 disabled:cursor-not-allowed shadow-lg text-sm sm:text-base"
              >
                {isUploading ? (
      ...
      {showQRZoom && qrCodeUrl && (
        <div
          className="fixed inset-0 z-[120] flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-md"
