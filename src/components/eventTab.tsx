...
export default function EventTab({ token, events }: { token: string; events: any[] }) {
  ...
  return (
    <div className="bg-black text-yellow-400 min-h-screen p-6 font-mono">
      ...
      {events.length === 0 ? (
        <div className="text-yellow-400 text-center mt-4">No events available.</div>
      ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <div className="bg-[#0a0a0a] border border-yellow-500 rounded-2xl p-6 w-[90%] max-w-xl shadow-2xl">
            ...
            <div className="space-y-3">
              {[1, 2, 3].map((n) => (
                <div key={n}>
                  <h3>{`Link ${n}: ${eventDetails[`link${n}`]}`}</h3>
                  <input
                    type="text"
                    placeholder={`New Link ${n}`}
                    onChange={(e) => {
                      if (n === 1) setLink1(e.target.value);
                      if (n === 2) setLink2(e.target.value);
                      if (n === 3) setLink3(e.target.value);
                    }}
